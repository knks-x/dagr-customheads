...
from dagr.data.ball_real import BallRealDataset
...
def train(loader: DataLoader,
    ...
    for i, data in enumerate(tqdm.tqdm(loader, desc=f"Training {run_name}")):               #tqdm: warps the trainloader and displays training progress in terminal. i=num of batch, data=num of sample
        ...
        if i % 10 == 0:                                                                    # only every 100th batch 
            ...
            save_path=f"/data/sbaumann/nature-gnn/dagr/visualize-to-debug/real/loaded_events+gt/{img_counter}"                                           #take sample with idx 0 from the current batch
...
if __name__ == '__main__':
    ...
    train_dataset = BallRealDataset(root=dataset_path, split="train", transform=None, debug=False, 
                         ball_diameter_min=ball_diameter_min, ball_diameter_max=ball_diameter_max, split_dir=None, crop_to_dsec=args.crop_to_dsec)
    val_dataset = BallRealDataset(root=dataset_path, split="val", transform=None, debug=False,
                        ball_diameter_min=ball_diameter_min, ball_diameter_max=ball_diameter_max, split_dir=None, crop_to_dsec=args.crop_to_dsec)

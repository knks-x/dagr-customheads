...
class DSECDet:
    def __init__(self, root: Path, split: str="train", sync: str="back", debug: bool=False, split_config=None):
        ...
        for f in self.subsequence_directories:
            ...
            self.img_idx_track_idxs[f.name] = compute_img_idx_to_track_idx(directory.tracks.tracks['t'],        #f.name=sequence name
                                                                           directory.images.timestamps)         #maps images and detections: which detections belong to each image within a sequence
    ...
    def get_index_window(self, index, num_idx, sync="back"):               
        if sync == "front":‚
            assert 0 < index < num_idx                                      #checks if index to >0, bc first image frame not used 
            i_0 = index - 1                                                 #sets event frame to one timestamp before set image frame (taking past events = history)
            i_1 = index
        else:
    ...
    def rel_index(self, index, directory_name=None):
        if directory_name is not None:      
            img_idx_to_track_idx = self.img_idx_track_idxs[directory_name]                  ##ccomput mapping for this sequence
            directory = self.directories[directory_name]
            return index, img_idx_to_track_idx, directory                                   #returns image index, mapping and directory for that sequence

...
def to_dense(self, x, pos, pooling, batch=None, batch_size=None):
    ...
    #print("Shapes:", x.shape, est_x.shape, est_y.shape, batch.shape)
    #print("Max est_x:", est_x.max().item(), "Max est_y:", est_y.max().item())
    #print("Max batch:", batch.max().item(), "Dense shape:", dense.shape)
...
class SplineConvToDense(MySplineConv):
    ...
    def to_dense(self, x, pos, pooling, batch=None, batch_size=None):
        ...
        #print("pos min/max x:", pos[:,0].min().item(), pos[:,0].max().item())
        #print("pos min/max y:", pos[:,1].min().item(), pos[:,1].max().item())
        #print("pooling:", pooling.detach().cpu().numpy())

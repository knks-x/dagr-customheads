...
def convert_to_evaluation_format(data):
    """After bbox in corner format: [x_min, y_min, x_min + w, y_min + h]"""
    targets = []
...
def postprocess_network_output(prediction, num_classes, conf_thre=0.01, nms_thre=0.65, height=640, width=640, filtering=True):
    prediction[..., :2] -= prediction[...,2:4] / 2 # cxcywh->xywh   converting cformat of bbox coordinates
    prediction[..., 2:4] += prediction[...,:2]

    output = []
    for i, image_pred in enumerate(prediction):                                                 #image_pred = one prediction per image with shape [num_anchors, 5]

        # If none are remaining => process next image
        if len(image_pred) == 0:
            device = prediction.device
            output.append({
                #append empty boxes, scores, labels of correct shape
                "boxes": torch.zeros(0, 4, dtype=torch.float32, device=device),                 
                "scores": torch.zeros(0, dtype=torch.float, device=device),
                "labels": torch.zeros(0, dtype=torch.long, device=device)                       #Even in class-agnostic detection, "labels" is still required for COCOeval
            })
            continue

         
        # Detections ordered as (x1, y1, x2, y2, obj_conf, class_id)  
        """Classification task: all classes get a value and the id of the class with best value is taken and saved as class_pred
                and additionally the class confidence, or the class confidence gets merged with object conf into one final score.
            Detection task: Only one class exists bc Coco eval needs one. Actually don't need to cut image_pred at all.   """
        
        #conf_mask = (image_pred[:, 4] >= conf_thre).squeeze()                                    #conf mask only objectness 
        conf_mask = (image_pred[:, 4] >= conf_thre) & (image_pred[:, 4] != 0.5)
        conf_mask = conf_mask.squeeze()
        detections = torch.cat((image_pred[:, :5],                                               #Coco eval needs a class - set class_id to 0
                        torch.zeros((image_pred.size(0), 1), device=image_pred.device)), 1)

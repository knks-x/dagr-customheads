...
def filter_tracks(dataset, image_width, image_height, class_remapping, min_bbox_height=0, min_bbox_diag=0, scale=1, only_perfect_tracks=False):
    ...
    for directory_path in dataset.subsequence_directories:
        ...
        # 2. stores image/frame pairs of two consecutive frames 
        #    (after filtering the tracks (and therefore frames) some images filtered out: indices: 0,1,2,5: pair(0,1) ok, pair(2,5) not ok (not consecutive))
        valid_image_index_pairs = construct_pairs(valid_image_indices, 2)

        #skip frame with index 0 because there are no preceding events to predict the frame for front mode
        if dataset.sync=="front":
            valid_image_index_pairs = np.array([pair for pair in valid_image_index_pairs if 0 not in pair])

        if only_perfect_tracks:
            #give timestamps for every pair-> (ts[i], ts[i+1]
            valid_image_timestamp_brackets = image_timestamps[valid_image_index_pairs]

            #checks which for each valid_ts_bracket which tracks belong there
            ...
            #finally returns perfect frame pairs (2,3)
            valid_image_index_pairs = valid_image_index_pairs[mask]

        #valid image pairs after filtering keyed by seqeuence
        image_index_pairs[directory_path.name] = valid_image_index_pairs
        #valid tracks after filtering keyed by sequence
...
def compute_class_mapping(classes, all_classes, mapping):
    """ 
        ...
        classes=object
        all_classes = ped, rider, car, ...
        mapping= ped, car, truck bus -> object; rest -> None
    """
    output_mapping = []
    for i, c in enumerate(all_classes):
        mapped_class = mapping[c]
        if mapped_class==None:
            print(F"Ignore class {c} wiht index {i}.")
            output_mapping.append(-1)
        else:
            if mapped_class in classes:
                output_mapping.append(classes.index(mapped_class))
    return np.array(output_mapping)

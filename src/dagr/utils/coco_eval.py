...
def _convert_to_coco_format(gt_boxes_list: List[Dict[str, Tensor]],
    ...
    #TODO: remove classification task -> all adjustement in _to_coco_format
...
def _coco_eval(dataset, results, num_gts):
    ...
    #TODO remove classification task : evaluation via COCOeval 
...
def _to_coco_format(gts, detections, categories, height=240, width=304):
    ...
    for image_id, (gt, pred) in enumerate(zip(gts, detections)):
        ...
        #every gt and pred box gets class_id is saved for further evaluation
        for bbox in gt:
            x1, y1 = bbox['x'], bbox['y']
            w, h = bbox['w'], bbox['h']
            area = w * h
        ...
        for bbox in pred:
            ...
            image_result = {
                'image_id': im_id,
                'category_id': 1,
                'score': float(bbox['class_confidence']),                                           #class confidence is obj_conf in 1-class detection, see "to_prophese-function"
                'bbox': [bbox['x'], bbox['y'], bbox['w'], bbox['h']],
            }
            results.append(image_result)

    #print unique gt and pred categories:    
    gt_unique = np.unique([a['category_id'] for a in annotations]) if len(annotations) > 0 else []
    pred_unique = np.unique([r['category_id'] for r in results]) if len(results) > 0 else []
    print(f"Unique GT catgeories:{gt_unique}")
    print(f"Unique Prediciton catgeories:{pred_unique}")

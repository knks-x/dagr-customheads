...
class Checkpointer:
    ...
    def search_for_checkpoint(self, resume_checkpoint: Path, best=False):
        if resume_checkpoint.is_file():
            return resume_checkpoint  # directly use the file
        ...
        #this covers the case if resume_from_best=True
...
def set_up_logging_directory(dataset, task, output_directory, exp_name="temp"):        #prev was her exp_name="temp". no default here so that the exp_name is 
    project = "low_latency-dsec-detection2"

    output_directory = output_directory / dataset / task
    output_directory.mkdir(parents=True, exist_ok=True)

    wandb.init(project="low_latency-dsec-detection2", id=exp_name, entity="sonja_baumann-university-of-t-bingen", save_code=True, dir=str(output_directory), settings=wandb.Settings(init_timeout=240), resume="allow")
...
def log_bboxes(data: Batch,
    ...
    for b, datum in enumerate(data.to_data_list()):
        ...
        image = torch.cat([image, image], dim=1)  #doubles the image horizontally
        images.append(image)
...
def log_single_sample(data: Batch,
               targets: List[Dict[str, torch.Tensor]],
               detections: List[Dict[str, torch.Tensor]],
               class_names: List[str],
               bidx: int,
    ...
    if not isinstance(data, Batch):
        raise ValueError("Expected data to be a Batch object")

    gt_bbox = []
    det_bbox = []
    images = []

    datum = data.to_data_list()[0]
    image = visualize_events(datum)
    image = torch.cat([image, image], dim=1)    #doubles the image horizontally
    images.append(image)

    if len(detections) > 0:
        det = detections[0]     #just passing one sample
        det = torch.cat([det['boxes'], det['labels'].view(-1,1), det['scores'].view(-1,1)], dim=-1)
        det_bbox.append(det)
    ...
    pred_bbox = torch.cat(det_bbox) if det_bbox else torch.empty((0, 6))
    gt_bbox = torch.cat(gt_bbox) if gt_bbox else torch.empty((0, 6))
    images = torch.cat(images, dim=-1)

    gt_bbox[:,[0,2]] /= datum.width
    gt_bbox[:,[1,3]] /= (2 * datum.height)

    pred_bbox[:,[0,2]] /= datum.width
    pred_bbox[:,[1,3]] /= (2 * datum.height)

    image = __convert_to_wandb_data(images.detach().float().cpu(),                  
                                    gt_bbox.detach().cpu(),                     #gt red boxes
                                    pred_bbox.detach().cpu(),                   #pred blue boxes
                                    class_names)
...
def visualize_events(data: Data)->torch.Tensor:
    ...
    if hasattr(data, "image"):
        ...
        #added this bc of dtype mismatch when logging visualization to wandb
